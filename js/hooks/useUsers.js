    ...
    const deleteUser = async (id) => {
        try {
            ...
            const { data: projectsWithUser, error: projectsSelectError } = await supabase
                .from('projects')
                .select('id, stagiaires')
                .contains('stagiaires', [id]);
            ...
            if (projectsWithUser && projectsWithUser.length > 0) {
                for (const project of projectsWithUser) {
                    const updatedStagiaires = (project.stagiaires || []).filter(stagiaireId => stagiaireId !== id);
                    
                    const { error: projectUpdateError } = await supabase
                        .from('projects')
                        .update({ stagiaires: updatedStagiaires })
                        .eq('id', project.id);
