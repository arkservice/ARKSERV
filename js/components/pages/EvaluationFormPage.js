        ...
        React.createElement('div', {
            ...
            React.createElement('div', {
                ...
                React.createElement('div', { key: 'ref' }, [
                    ...
                    }, session.pdc.ref || `PDC #${session.pdc.pdc_number}`)
                ]),
...
function createSection02(formData, handleFieldChange, session) {
    const lieuFormation = session?.lieu_formation || '';

    // Conditions d'affichage basées sur le lieu de formation
    const isDansVosLocaux = lieuFormation === 'Dans vos locaux';
    const isDansNosLocaux = lieuFormation === 'Dans nos locaux';
    const isADistance = lieuFormation === 'À distance';

    // Questions à afficher selon le lieu
    const showLocaux = isDansNosLocaux; // Afficher uniquement si "Dans nos locaux"
    const showMateriel = isDansNosLocaux; // Afficher uniquement si "Dans nos locaux"
    const showFormationDistance = isADistance; // Afficher uniquement si "À distance"
    const showRepas = isDansNosLocaux; // Afficher uniquement si "Dans nos locaux"
