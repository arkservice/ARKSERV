...
async function generateEvaluationPDF(evaluationData, layoutParams = {}) {
    ...
    // === HEADER (0-22mm) - Réutilisé depuis PDC ===
    await renderSection1_Header(doc, params);

    // === TITRE ET INFOS (22-75mm) - SECTION OPTIMISÉE ===
    let yPos = 28;
    ...
    // Encadré informations - HAUTEUR RÉDUITE
    ...
    // Réutiliser les variables themes et formateurThemes déjà déclarées
    const themeKeys = Object.keys(themes).sort();
    ...
    themeKeys.forEach((themeKey, index) => {
        ...
        // Fond alterné - HAUTEUR RÉDUITE À 13mm
        if (index % 4 < 2) {
            doc.setFillColor(249, 250, 251);
            doc.rect(xStart, currentY - 2.5, colWidth, 13, 'F');
        }

        // Titre thème (taille de police réduite pour compacité)
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(...primaryColor);
        doc.setFontSize(7.5);
        ...
        currentY += 13; // HAUTEUR TOTALE RÉDUITE À 13mm
    ...
    // === FOOTER (260-297mm) - Réutilisé depuis PDC ===
    await renderSection5_Footer(doc, params);
...
// Export global
console.log('🚀 [DEBUG] pdfGenerator.js chargé - VERSION 5 SECTIONS + CONVOCATION + CONVENTION + ÉMARGEMENT + ÉVALUATION');
...
console.log('✅ [DEBUG] Générateurs PDF exportés:', {
    ...
    evaluation: typeof window.generateEvaluationPDF
});